  ...
  try {
    ...
    if (esVoz || esAudio) {
      ...
      try {
        ...
        // Telegram voice messages are always ogg/opus regardless of mime_type field
        const mimeType = esVoz ? "audio/ogg" : (audioObj.mime_type || "audio/ogg");
        ...
        if (!datos || !datos.esReserva) {
          await sendTelegram(chatId, `🤔 No he entendido el audio como una reserva.\n\nPrueba escribiéndome el mensaje o di algo como:\n<i>"Mesa para cuatro el sábado a las dos, a nombre de María"</i>`);
          return res.status(200).json({ ok: true });
        }
        await guardarReserva(chatId, datos);
      } catch(audioErr) {
        console.error("Error procesando audio:", audioErr);
        await sendTelegram(chatId, "❌ No he podido procesar el audio. Prueba escribiéndome la reserva como texto.");
      }
      return res.status(200).json({ ok: true });
    }
